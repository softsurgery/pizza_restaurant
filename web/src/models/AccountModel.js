    ...
    async createOrUpdateUserDetails() {
        ...
        try {
            ...
            if (response.data) {
                ...
                const updatedDetails = await axios.put(`/userDetails/${userId}`, userDetails);
                runInAction(() => {
                    // Update MobX state
            ...
            if (error.response && error.response.status === 404) {
                ...
                const createdDetails = await axios.post(`/userDetails`, { ...userDetails, userId });
                runInAction(() => {
                    // Update MobX state
                ...
                console.error("Error while creating/updating user details:", error.message);
                throw error;
            }
        }
    }
